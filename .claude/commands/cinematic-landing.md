...
This command builds a cinematic, fully-animated landing page from scratch. Unlike `/new-project` (which transforms the Snap boilerplate), this workflow scaffolds a **standalone Vite + React project** with GSAP animations, Tailwind CSS, and a curated design system.

## Agent Flow — MUST FOLLOW

When this command is invoked (or this file is loaded into a fresh project), immediately ask **exactly these questions** using AskUserQuestion in a single call, then build the full site from the answers. Do not ask follow-ups. Do not over-discuss. Build.

## Initialization Request

**ANALYZE** the request: "$ARGUMENTS"

If no arguments provided, begin with the Discovery Questions below.
...
1. STOP immediately
2. Report error clearly with step number
